...
public class FootBallView extends View {
    ...
    private Ball mBall;
    private Player[] mPlayerRed = new Player[5];
    private Player[] mPlayerBlue = new Player[5];
...
//                Toast.makeText(mContext, mBall.detect(goalBlue) ? "红队胜利" : "蓝队胜利", Toast.LENGTH_LONG).show();
    ...
    private void initPlayer(Canvas canvas) {
        ...
        mPlayerRed[0].cx = 10 + (mWidth - 20) / 6;
        mPlayerRed[0].cy = (mHeight - 20) / 8;
        mPlayerRed[0].widthLimit[0] = 10;
        mPlayerRed[0].widthLimit[1] = 10 + (mWidth - 20) / 3;

        mPlayerRed[1] = new Player();
        mPlayerRed[1].cx = 10 + (mWidth - 20) / 6 * 5;
        mPlayerRed[1].cy = (mHeight - 20) / 8;
        mPlayerRed[1].widthLimit[0] = 10 + (mWidth - 20) / 3 * 2;
        mPlayerRed[1].widthLimit[1] = mWidth - 10;

        mPlayerRed[2] = new Player();
        mPlayerRed[2].cx = 10 + (mWidth - 20) / 2;
        mPlayerRed[2].cy = (mHeight - 20) / 4;
        mPlayerRed[2].widthLimit[0] = 10 + (mWidth - 20) / 4;
        mPlayerRed[2].widthLimit[1] = 10 + (mWidth - 20) / 4 * 3;


        mPlayerRed[3] = new Player();
        mPlayerRed[3].cx = 10 + (mWidth - 20) / 6;
        mPlayerRed[3].cy = (mHeight - 20) / 8 * 3;
        mPlayerRed[3].widthLimit[0] = 10;
        mPlayerRed[3].widthLimit[1] = 10 + (mWidth - 20) / 3;

        mPlayerRed[4] = new Player();
        mPlayerRed[4].cx = 10 + (mWidth - 20) / 6 * 5;
        mPlayerRed[4].cy = (mHeight - 20) / 8 * 3;
        mPlayerRed[4].widthLimit[0] = 10 + (mWidth - 20) / 3 * 2;
        mPlayerRed[4].widthLimit[1] = mWidth - 10;

        for (int i = 0; i < 5; i++) {
            mPlayerRed[i].radius = (mHeight - 20) / 20;
            Paint paint = new Paint();
        ...
        mPlayerBlue[0].cx = 10 + (mWidth - 20) / 6;
        mPlayerBlue[0].cy = (mHeight - 20) / 8 * 7;
        mPlayerBlue[0].widthLimit[0] = 10;
        mPlayerBlue[0].widthLimit[1] = 10 + (mWidth - 20) / 3;

        mPlayerBlue[1] = new Player();
        mPlayerBlue[1].cx = 10 + (mWidth - 20) / 6 * 5;
        mPlayerBlue[1].cy = (mHeight - 20) / 8 * 7;
        mPlayerBlue[1].widthLimit[0] = 10 + (mWidth - 20) / 3 * 2;
        mPlayerBlue[1].widthLimit[1] = mWidth - 10;

        mPlayerBlue[2] = new Player();
        mPlayerBlue[2].cx = 10 + (mWidth - 20) / 2;
        mPlayerBlue[2].cy = (mHeight - 20) / 8 * 6;
        mPlayerBlue[2].widthLimit[0] = 10 + (mWidth - 20) / 4;
        mPlayerBlue[2].widthLimit[1] = 10 + (mWidth - 20) / 4 * 3;

        mPlayerBlue[3] = new Player();
        mPlayerBlue[3].cx = 10 + (mWidth - 20) / 6;
        mPlayerBlue[3].cy = (mHeight - 20) / 8 * 5;
        mPlayerBlue[3].widthLimit[0] = 10;
        mPlayerBlue[3].widthLimit[1] = 10 + (mWidth - 20) / 3;

        mPlayerBlue[4] = new Player();
        mPlayerBlue[4].cx = 10 + (mWidth - 20) / 6 * 5;
        mPlayerBlue[4].cy = (mHeight - 20) / 8 * 5;
        mPlayerBlue[4].widthLimit[0] = 10 + (mWidth - 20) / 3 * 2;
        mPlayerBlue[4].widthLimit[1] = mWidth - 10;
        for (int i = 0; i < 5; i++) {
            mPlayerBlue[i].radius = (mHeight - 20) / 20;
            Paint paint = new Paint();
    ...
    private void initScene(Canvas canvas) {
        ...
        for (int i = 0; i < 18; i += 2) {
            points.add(new Point(10, 10 + i / 2 * (mHeight - 20) / 8));
            points.add(new Point(mWidth - 10, 10 + i / 2 * (mHeight - 20) / 8));
            canvas.drawLine(10, 10 + i / 2 * (mHeight - 20) / 8, mWidth - 10, 10 + i / 2 * (mHeight - 20) / 8, paint);
        }
        canvas.drawLine(10, 10, 10, mHeight - 10, paint);
        canvas.drawLine(mWidth - 10, 10, mWidth - 10, mHeight - 10, paint);

        goalRed.cx = (mWidth - 20) / 2;
        goalRed.width = (mWidth - 20) / 5;
        goalRed.height = (mHeight - 20) / 25;
        goalRed.cy = 10 + goalRed.height / 2;
        ...
        goalBlue.cx = (mWidth - 20) / 2;
        goalBlue.width = (mWidth - 20) / 5;
        goalBlue.height = (mHeight - 20) / 25;
        goalBlue.cy = (mHeight - 10) - goalBlue.height / 2;
